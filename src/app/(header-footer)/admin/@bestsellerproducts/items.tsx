...
export default function BestSellerProductsComponents() {
  ...
  return (
    <div className="mt-8">
      <div className="flex justify-between items-start gap-2">
        <h2 className="mb-4 text-2xl">Best Seller items list...</h2>
        <Button variant="outline" onClick={fetchBestSellerItems} disabled={loading}>Refresh List</Button>
      </div>
      {
        loading ? (
          <div>
...
function Item({ item }: { item: string }) {
  ...
  const handleRemove = async () => {
    ...
    setDeleting(true)
    const response = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/admin/remove-bestseller-item`, {
      method: 'DELETE',
  ...
  return (
    <div className={`p-4 rounded-xl shadow-lg${deleting ? ' opacity-50' : ''}`}>
      ...
      <p className="text-sm">
        <span className="inline-block max-w-[46%] truncate my-1 px-4 py-1 bg-neutral-800 text-neutral-100 rounded-full mr-1">{CategoryMap.get(product?.category ?? '')}</span>
        <span className="inline-block max-w-[46%] truncate my-1 px-4 py-1 bg-neutral-800 text-neutral-100 rounded-full">{SubCategoryMap.get(product?.subcategory ?? '')}</span>
      </p>
      <button onClick={e => handleRemove()} className="mt-2 py-1.5 w-full text-sm flex justify-center items-center text-red-600 border-2 border-red-500 hover:text-neutral-100 hover:bg-red-500 rounded transition">
        <FaTrash /> Delete from best seller
      </button>
    </div>
  )
}