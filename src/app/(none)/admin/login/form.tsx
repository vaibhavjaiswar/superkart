...
export default function AdminLoginForm() {
  ...
  const [email, setEmail] = useState('admin@superkart.com')
  const [password, setPassword] = useState('administratorofsuperkart')
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isFormValid) {
      ...
      if (json.ok) {
        ...
        dispatch(setAdmin({ email: admin.email }))
        router.push('/admin')
      } else {
  ...
  const validateFields = () => {
    ...
    if (password.length < 8) {
      setPasswordFieldInfo('Password length should be minimum 8 characters.')
