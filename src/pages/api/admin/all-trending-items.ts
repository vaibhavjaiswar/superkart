...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ResponseType>) {
  try {
    ...
    const results = await commonCollection.findOne({ 'label': 'trending' })

    if (results) {
      res.status(200).json({ ok: true, message: 'Trending items fetched.', data: results?.items || [] })
    } else {
      res.status(200).json({ ok: false, message: 'Some issue occured while fetching trending items.' })
    }
