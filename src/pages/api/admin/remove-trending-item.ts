...
export default async function handler(req: NextApiRequest, res: NextApiResponse<ResponseType>) {
  try {
    ...
    const result = await commonCollection.findOne({ 'label': 'trending' })
    ...
    if (items.includes(productID)) {
      ...
      if (acknowledged && modifiedCount === 1 && matchedCount === 1) {
        res.status(200).json({ ok: true, message: 'Product deleted from trending items.' })
      } else {
        res.status(400).json({ ok: true, message: 'Error while removing item.' })
      }
    } else {
      res.status(400).json({ ok: false, message: 'Product ID is not in trending items.' })
    }
